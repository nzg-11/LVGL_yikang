...
void ui_user_permission_create(lv_obj_t *user_manage_scr)
{
    ...
    if(family_menber_scr == NULL) {
        family_menber_scr = lv_obj_create(NULL);  // 创建独立屏幕
        lv_obj_add_event_cb(family_menber_scr, user_permission_scr_load_cb, LV_EVENT_SCREEN_LOAD_START, NULL);
    } else {
        lv_obj_clean(family_menber_scr);          // 清空原有内容
    }
    ...
    lv_obj_add_style(family_menber_scr, &user_permission_grad_style, LV_STATE_DEFAULT);// 应用渐变样式到屏幕

    // 左上角返回按钮
    lv_obj_t *back_btn = create_image_obj(family_menber_scr, "H:back.png", 52, 123);
